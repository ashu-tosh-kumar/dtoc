...
(() => {
  ...
  function updateSetting(storageKey, value) {
    chrome.storage.local.get(['siteSettings'], (result) => {
      ...
      if (hasSiteOverride && (storageKey === 'pinned' || storageKey === 'minimized' || storageKey === SETTINGS_KEY.CLOSED)) {
        ...
        if (storageKey === 'pinned') {
          chrome.storage.local.set({ pinned: value, minimized: !value }, () => {
            applyStateToUI();
          });
        } else if (storageKey === 'minimized') {
          chrome.storage.local.set({ minimized: value, pinned: !value }, () => {
            applyStateToUI();
          });
        } else {
  ...
  function setActiveHeading(activeId) {
    ...
    if (activeId === '' || activeId === null) {
      ...
      const activeLink = shadowRoot.querySelector(`.toc-link[href="#${escapeCSSSelector(activeId)}"]`);
      if (activeLink) {
      ...
      const activeNotch = shadowRoot.querySelector(`.dtoc-notch[data-id="${escapeCSSSelector(activeId)}"]`);
      if (activeNotch) {
